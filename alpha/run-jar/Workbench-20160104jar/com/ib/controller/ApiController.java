...
/* Modified by PA since it was such a crappy design - 2014 */
...
public class ApiController implements EWrapper {
	...
	public ApiController() {
		m_inLogger = new ILogger() {
			@Override
			public void log(String valueOf) {
				// nothing by design
			}
		};
		m_outLogger = new ILogger() {
			@Override
			public void log(String valueOf) {
				// nothing by design
			}
		};
		m_client = new ApiConnection( this, m_inLogger, m_outLogger);
	}
	...
	// Added by PA
	public synchronized int getNextValidOrderIdAndIncrement() {
		return m_orderId++;
	}
	// Added by PA
	public int getNextValidReqId() {
		return m_reqId;
	}
	
	
	@Override public void nextValidId(int orderId) {
		System.out.println("Setting next valid order ID to:" + orderId);
		m_orderId = orderId;
		System.out.println("Setting next valid request ID to:" + (orderId+10000000));
	...
	public void placeOrModifyOrder(NewContract contract, final NewOrder order, final IOrderHandler handler) {
		...
		if (order.orderId() == 0) {
			order.orderId( getNextValidOrderIdAndIncrement() );
			if (handler != null) {
    ...
    /// HACK!!!!  OTHEWISE WE CAN NEVER SET m_include_expired!
    
    
	/** @param endDateTime format is YYYYMMDD HH:MM:SS [TMZ]
	 *  @param duration is number of durationUnits */
    public void reqHistoricalData2(Contract contract, String endDateTime, int duration, DurationUnit durationUnit, BarSize barSize, WhatToShow whatToShow, boolean rthOnly, IHistoricalDataHandler handler) {
    	int reqId = m_reqId++;
    	m_historicalDataMap.put( reqId, handler);
    	String durationStr = duration + " " + durationUnit.toString().charAt( 0);
    	m_client.reqHistoricalData(reqId, contract, endDateTime, durationStr, barSize.toString(), whatToShow.toString(), rthOnly ? 1 : 0, 2);
		sendEOM();
    }
